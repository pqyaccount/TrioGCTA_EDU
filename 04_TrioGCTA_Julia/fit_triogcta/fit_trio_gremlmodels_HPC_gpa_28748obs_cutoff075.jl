...
function sel_trio(A, m_inds, f_inds, c_inds)
     # Check for relatives, threshold for GRM = 0.075
    mf_inds = vcat(m_inds, f_inds)
    A_mf = A[mf_inds, mf_inds]
    keep_A_mf = kinship_pruning(A_mf, method=:bottom_up, cutoff=0.075)
...
# --------------------------------------
	...
	println("Creating GREML model object")
	
	# The optimization must converge completely:
	m_full.opt.ftol_abs = 0
	m_full.opt.ftol_rel = 0
	# println("Model created. Setting convergence thresholds")
	
    @time GREMLModels.fit!(m_full)
	println("Fitting Full")
	
	m_full.δ
	println("Outputting variance components")
	
	filename = vari * "_" * threshold * "_full_$(size(mobadat, 1)).jld"
	println(filename)

	if Sys.iswindows()
	JLD.save("M:\\p805-qiyuanp\\TrioGCTA\\Step05_TrioGCTA_Julia\\results\\075\\" * filename, "model", m_full)
	else
    JLD.save("/tsd/p805/home/p805-qiyuanp/TrioGCTA/Step05_TrioGCTA_Julia/results/075" * filename, "model", m_full)
	end
...
# -------------------------------------------
	...
	if Sys.iswindows()
	JLD.save("M:\\p805-qiyuanp\\TrioGCTA\\Step05_TrioGCTA_Julia\\results\\075\\" * filename, "model", m_mo)
	else
...
# -------------------------------------------
	...
	if Sys.iswindows()
	JLD.save("M:\\p805-qiyuanp\\TrioGCTA\\Step05_TrioGCTA_Julia\\results\\075\\" * filename, "model", m_fo)
	else
...
# -------------------------------------------
	...
	if Sys.iswindows()
	JLD.save("M:\\p805-qiyuanp\\TrioGCTA\\Step05_TrioGCTA_Julia\\results\\075\\" * filename, "model", m_mf)
	else
...
# -------------------------------------------
	...
	if Sys.iswindows()
	JLD.save("M:\\p805-qiyuanp\\TrioGCTA\\Step05_TrioGCTA_Julia\\results\\075\\" * filename, "model", m_indep)
	else
...
# -------------------------------------------
	...
	if Sys.iswindows()
	JLD.save("M:\\p805-qiyuanp\\TrioGCTA\\Step05_TrioGCTA_Julia\\results\\075\\" * filename, "model", m_direct)
	else
...
function findGRMs(A, m_inds_use, f_inds_use, c_inds_use) 
	...
	if Sys.iswindows()
	JLD.save("M:\\p805-qiyuanp\\TrioGCTA\\Step05_TrioGCTA_Julia\\results\\075\\" * filename, "model", m_rdr)
	else
    JLD.save("/tsd/p805/home/p805-qiyuanp/TrioGCTA/Step05_TrioGCTA_Julia/results/075/" * filename, "model", m_rdr)
	end
