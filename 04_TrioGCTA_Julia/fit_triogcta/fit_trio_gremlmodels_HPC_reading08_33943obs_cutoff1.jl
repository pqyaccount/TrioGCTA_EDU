...
if Sys.iswindows()
	mobadat = DataFrame(CSV.File("M:\\p805-qiyuanp\\TrioGCTA\\data\\MoBaPsychGen_v1_1m_reading08_33943obs.moba", missingstring = "NA"))
else
	mobadat = DataFrame(CSV.File("/tsd/p805/home/p805-qiyuanp/TrioGCTA/data/MoBaPsychGen_v1_1m_reading08_33943obs.moba", missingstring = "NA"))
end
...
# -------------------------------------------
    ...
    @time m_indep = GREMLModel(dat, ini, lbs, false)
		# The optimization must converge completely:
		m_indep.opt.ftol_abs = 0
		m_indep.opt.ftol_rel = 0
    @time fit!(m_indep)
...
# -------------------------------------------
	...
	# The optimization must converge completely:
	m_direct.opt.ftol_abs = 0
	m_direct.opt.ftol_rel = 0
    @time fit!(m_direct)
	m_direct.δ
...
function findGRMs(A, m_inds_use, f_inds_use, c_inds_use) 
	...
	@time m_rdr = GREMLModel(dat, ini, lbs, false)
		# The optimization must converge completely:
		m_rdr.opt.ftol_abs = 0
		m_rdr.opt.ftol_rel = 0
    @time fit!(m_rdr)
	m_rdr.δ
