...
function sel_trio(A, m_inds, f_inds, c_inds)
     # Check for relatives, threshold for GRM = 0.10
...
if Sys.iswindows()
	mobadat = DataFrame(CSV.File("M:\\p805-qiyuanp\\TrioGCTA\\data\\MoBaPsychGen_v1_1m_english08_33784obs.moba", missingstring = "NA"))
else
	mobadat = DataFrame(CSV.File("/tsd/p805/home/p805-qiyuanp/TrioGCTA/data/MoBaPsychGen_v1_1m_english08_33784obs.moba", missingstring = "NA"))
end
...
# --------------------------------------
	...
	# Convergence criteria: Tested thresholds from 0 to 1e-16 (e.g., 1e-12, 1e-14).
  # English 08 results consistently show singular fits/boundary solutions regardless of precision.
  # For detailed statistical justification and model selection, see Supplementary Materials.
	m_full.opt.ftol_abs = 1.0e-12
	m_full.opt.ftol_rel = 1.0e-12
...
# -------------------------------------------
	...
	# The optimization must converge completely:
	m_mo.opt.ftol_abs = 1.0e-12
	m_mo.opt.ftol_rel = 1.0e-12
	@time fit!(m_mo)
	println("Fitting MO")
	m_mo.δ
...
# -------------------------------------------
	...
	# The optimization must converge completely:
	m_fo.opt.ftol_abs = 1.0e-12
	m_fo.opt.ftol_rel = 1.0e-12
	@time fit!(m_fo)
	println("Fitting FO")
	m_fo.δ
...
# -------------------------------------------
	...
	# The optimization must converge completely:
	m_mf.opt.ftol_abs = 1.0e-12
	m_mf.opt.ftol_rel = 1.0e-12
	@time fit!(m_mf)
	println("Fitting MF")
	m_mf.δ
...
# -------------------------------------------
	...
	# The optimization must converge completely:
	m_indep.opt.ftol_abs = 1.0e-12
	m_indep.opt.ftol_rel = 1.0e-12
    @time fit!(m_indep)
...
# -------------------------------------------
	...
	# The optimization must converge completely:
	m_direct.opt.ftol_abs = 1.0e-12
	m_direct.opt.ftol_rel = 1.0e-12
    @time fit!(m_direct)
	m_direct.δ
...
function findGRMs(A, m_inds_use, f_inds_use, c_inds_use) 
	...
	# The optimization must converge completely:
	m_rdr.opt.ftol_abs = 1.0e-12
	m_rdr.opt.ftol_rel = 1.0e-12
    @time fit!(m_rdr)
	m_rdr.δ
